...
public class Server {
	...
	public Server() {
		...
		class graphicDisplay extends Frame implements WindowListener, KeyListener {
			...
			public graphicDisplay() {
				...
				serverDisplay.setBorder(new javax.swing.border.TitledBorder(null,
						"Server is running", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
						javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font(
								"Tahoma", 1, 16), new java.awt.Color(0, 102, 102)));
			...
			public void keyTyped(KeyEvent e) {
				if (e.getKeyChar() == '\n' && filetext.isEditable()) {
					...
					if (!isIntNumber(filetext.getText())) {
						...
						if (clients < 3) {
							updateGUI(Color.RED,
									"You gave invalid clients's number.\nThe default number (3) will be used\n");
							clients = 3;
						} else
	...
	public void runServer() {
		...
		try {
			...
			if (!graphics) {
				...
				try {
					String sclients = in.readLine();
					clients = Integer.parseInt(sclients);
				} catch (Exception e) {
				...
				// Synchronized the server with the key event below
			...
			*/
				...
				public void run() {
					try {
						while (true) {
							...
							updateGUI(new java.awt.Color(0, 202, 202),
			...
			while (true) {
				...
				while (ClientList.size() < clients && !Server.Trajset) {
					try {
						if (DEBUG)
							System.out.println("(countClients < clients)Barrier:"
				...
				// System.out.println("countUBClients in server=" +
				// countUBClients);
				// System.out.println("Is trajectory set = " + Trajset);
				// System.out.println("UBtable.size() = " + UBList.size());
				// This is a barrier if client searcher is the first one
				while (countUBCalls < ClientList.size() - 1) {
					try {
						if (DEBUG)
							System.out.println("(countUBClients < ClientList.size()-1)Barrier:"
									+ countUBCalls + "<" + (ClientList.size() - 1));
						Thread.sleep(interval);
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
				...
				while (UBList.size() < countUBCalls) {
					try {
						if (DEBUG)
							System.out.println("(UBtable.size() < countUBClients)Barrier:"
									+ UBList.size() + "<" + countUBCalls);
						Thread.sleep(interval);
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
				...
				// biggest UB
				// for (ThreadUB tub : UBList) {
				// System.out.println(tub.thread.getId() + "." + tub.thread.usr
				// + ".UB = "
				// + tub.UBLCSS);
				// }
				for(int i=0;i<K;i++){
				synchronized (UBList) {
					if (UBList.size() > i) {
						...
						synchronized (Server.countLCSSCalls) {
							...
							// System.out.println("(" + Server.countLCSSCalls +
							// ")Server.countLCSSCalls");
						}
					}else
						break;
				...
				while (LCSSList.size() < clients - 1 && ClientList.size() > clients - 1) {
					try {
						if (DEBUG)
							System.out
									.println("(LCSSList.size() < 2 && ClientList.size() > 2)Barrier:"
											+ LCSSList.size()
											+ "<2 && "
											+ ClientList.size()
											+ "> 2");
				...
				// System.out.println("LCSSthread.size(" + LCSSthread.size() +
				// ") ");
				// wait until all the LCSS calls are answered
				while (LCSSList.size() < countLCSSCalls && UBList.size() > 2) {
					try {
						if (DEBUG)
							System.out.println("(LCSSthread.size() < countLCSSCalls) Barrier:"
									+ LCSSList.size() + "<" + countLCSSCalls);
						Thread.sleep(interval);
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
				...
				if (LCSSList.size() > 0) {
					...
					updateGUI(Color.BLACK, " has the most similar trajectory with you with ip "
							+ LCSSList.get(0).thread.ip + "\n");
	...
	public static void initPrams() {
		try {
			FileInputStream fileInputStream = new FileInputStream("./Wifi.cfg");
